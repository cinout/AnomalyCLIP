...
class MuSc:
    ...
    def load_datasets(self, category, divide_num=1, divide_iter=0):
        # dataloader
        if self.dataset == "visa":
            test_dataset = visa.VisaDataset(
                source=self.path,
                split=visa.DatasetSplit.TEST,
                classname=category,
        ...
        if self.vis_type == "single_norm":
            ...
            for i, path in enumerate(image_path_list):
                ...
                if anomaly_type not in ["good", "Normal", "ok"] and gt_list[i] != 0:
                    ...
                    anomaly_map = cv2.applyColorMap(
                        anomaly_map.astype(np.uint8), cv2.COLORMAP_JET
                    )
    ...
    def make_category_data(self, category):
        ...
        for divide_iter in range(divide_num):
            test_dataset = self.load_datasets(
                category, divide_num=divide_num, divide_iter=divide_iter
            )
            ...
            # TODO: start to ...
            # extract features
            patch_tokens_list = []  # each element is an batch's images' patch features
            ...
            for image_info in tqdm(test_dataloader):
                ...
                with torch.no_grad(), torch.cuda.amp.autocast():
                    ...
                    if "dinov2" in self.model_name:
                        ...
                        fake_cls = [
                            torch.zeros_like(p)[:, 0:1, :] for p in patch_tokens
                        ]
                        patch_tokens = [
                        ...
                        patch_tokens_all = self.dino_model.get_intermediate_layers(
                            x=input_image, n=max(self.features_list)
                        )
                        image_features = self.dino_model(input_image)
                        patch_tokens = [
                            patch_tokens_all[l - 1].cpu() for l in self.features_list
                        ]
                    else:
                        # TODO: get CLIP features
                        image_features, patch_tokens = self.clip_model.encode_image(
                            input_image, self.features_list
                        )
            ...
            end_time = time.time()
            print(
                "extract time: {}ms per image".format(
                    (end_time - start_time) * 1000 / subset_num
                )
            )
            ...
            for r in self.r_list:
                ...
                for im in range(len(patch_tokens_list)):
                    patch_tokens = [
                        p.to(self.device) for p in patch_tokens_list[im]
                    ]  # 4* [4, 1370, 1024]

                    with torch.no_grad(), torch.cuda.amp.autocast():
                        features = LNAMD_r._embed(
                            patch_tokens
                        )  # (B=4, L=1369, layer=4, C=1024)
                ...
                end_time = time.time()
                print(
                    "LNAMD-{}: {}ms per image".format(
                        r, (end_time - start_time) * 1000 / subset_num
                    )
                )
                ...
                end_time = time.time()
                print(
                    "MSM: {}ms per image".format(
                        (end_time - start_time) * 1000 / subset_num
                    )
                )
            anomaly_maps_iter = torch.mean(anomaly_maps_r, 0).to(
                self.device
            )  # [N=#test, L=1369]
            del anomaly_maps_r
            torch.cuda.empty_cache()
    ...
    def main(self):
        ...
        for category in self.categories:
            image_metric, pixel_metric = self.make_category_data(
                category=category,
            )
            auroc_sp, f1_sp, ap_sp = image_metric
            auroc_px, f1_px, ap_px, aupro = pixel_metric
                ...
                for row_index in range(len(self.categories)):
                    if col_index == 0:
                        sheet.cell(
                            row=row_index + 2,
                            column=col_index + 1,
                            value=self.categories[row_index],
                        )
                    else:
                        sheet.cell(
                            row=row_index + 2,
                            column=col_index + 1,
                            value=auroc_px_ls[row_index] * 100,
                        )
                        sheet.cell(
                            row=row_index + 2,
                            column=col_index + 2,
                            value=f1_px_ls[row_index] * 100,
                        )
                        sheet.cell(
                            row=row_index + 2,
                            column=col_index + 3,
                            value=ap_px_ls[row_index] * 100,
                        )
                        sheet.cell(
                            row=row_index + 2,
                            column=col_index + 4,
                            value=aupro_ls[row_index] * 100,
                        )
                        sheet.cell(
                            row=row_index + 2,
                            column=col_index + 5,
                            value=auroc_sp_ls[row_index] * 100,
                        )
                        sheet.cell(
                            row=row_index + 2,
                            column=col_index + 6,
                            value=f1_sp_ls[row_index] * 100,
                        )
                        sheet.cell(
                            row=row_index + 2,
                            column=col_index + 7,
                            value=ap_sp_ls[row_index] * 100,
                        )
                    if row_index == len(self.categories) - 1:
                        if col_index == 0:
                            sheet.cell(
                                row=row_index + 3, column=col_index + 1, value="mean"
                            )
                        else:
                            sheet.cell(
                                row=row_index + 3,
                                column=col_index + 1,
                                value=auroc_px_mean * 100,
                            )
                            sheet.cell(
                                row=row_index + 3,
                                column=col_index + 2,
                                value=f1_px_mean * 100,
                            )
                            sheet.cell(
                                row=row_index + 3,
                                column=col_index + 3,
                                value=ap_px_mean * 100,
                            )
                            sheet.cell(
                                row=row_index + 3,
                                column=col_index + 4,
                                value=aupro_mean * 100,
                            )
                            sheet.cell(
                                row=row_index + 3,
                                column=col_index + 5,
                                value=auroc_sp_mean * 100,
                            )
                            sheet.cell(
                                row=row_index + 3,
                                column=col_index + 6,
                                value=f1_sp_mean * 100,
                            )
                            sheet.cell(
                                row=row_index + 3,
                                column=col_index + 7,
                                value=ap_sp_mean * 100,
                            )
